...
namespace Business.Concrete
{
    public class RentalManager : IRentalService
    {
        ...
        public IDataResult<List<Rental>> GetByYacthId(int yacthId)
        {
            List<Rental> rentals = _rentalDal.GetAll(c => c.YacthId == yacthId);
            if (rentals == null)
            {
                return new ErrorDataResult<List<Rental>>(Messages.GetAllRental);
            }
        ...
        {
            IResult result = BusinessRules.Run(CheckIfReturnDateNull(rental.YacthId));
            if (result != null)
            {
                return result;
            }
        ...
        private IResult CheckIfReturnDateNull(int yacthId)
        {
            var result = _rentalDal.GetAll(r => r.YacthId == yacthId && r.ReturnDate == null);
            if (result.Count > 0)
            {
        ...
        public IDataResult<List<RentalDetailDto>> GetRentalDetailsByCustomerId(int customerId)
        {
            ...
            if (rentals == null)
            {
                return new ErrorDataResult<List<RentalDetailDto>>(Messages.NoRentAYacth);
            }
